                    ...
                    <form action="{{ route('dashboard.roles.update', ["role" => $role->id]) }}" method="post">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" id="roleName" class="form-control @error('name') is-invalid @enderror" placeholder="Role Name" value="{{ old('name', $role->name) }}">
                                @error('name')
                            ...
                            <div class="form-group">
                                ...
                                <select class="roles @error('role_permissions') is-invalid @enderror" multiple="multiple" data-placeholder="Select permissions" style="width: 100%;" name="role_permissions[]" id="role_permissions">
                                    @foreach ($permissions as $permission)
                                        <option value="{{ $permission->name }}" {{ in_array($permission->name, old('role_permissions', $rolePermissions->toArray())) ? 'selected="selected"' : '' }}>{{ $permission->name }}</option>
                                    @endforeach
                                </select>
